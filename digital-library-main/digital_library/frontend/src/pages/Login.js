...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await authService.login(form.email, form.password, form.rememberMe);
      if (response.success) {
        ...
        <div className="space-y-3">
          ...
          <motion.button
            ...
            {guestLoading ? (
              ...
              <span className="flex items-center gap-2"><FiPhone size={14} /> {t('guestSendCode')}</span>
            )}
          </motion.button>
        </div>
