...
import {
  FiGrid, FiSearch, FiUpload, FiBookmark, FiClock, FiHeadphones, FiVolume2,
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
      ...
      {lowLiteracy && (
        <div className="mx-3 mb-2 px-3 py-2 rounded-xl bg-brand-600/10 border border-brand-200 flex items-center gap-2">
          <FiVolume2 size={18} className="text-brand-600 flex-shrink-0" />
      ...
      <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map(({ icon: Icon, key, to, emoji }) => (
          <NavLink key={to} to={to}
            ...
            {({ isActive }) => (
              <>
                <Icon size={lowLiteracy ? 22 : 18} className={isActive ? 'text-white' : (lowLiteracy ? 'text-brand-600' : 'text-brand-500 group-hover:text-brand-600')} />
                <span>{t(key)}</span>
              </>
            )}
          </NavLink>
